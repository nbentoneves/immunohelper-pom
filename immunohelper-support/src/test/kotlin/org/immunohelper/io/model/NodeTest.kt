...
class NodeTest {
    ...
    @Test
    fun `Should create a tree cell using empty list of cells`() {
        ...
        assertNotNull(mainTree)
        assertEquals(0, mainTree.listOfChildren.size)

    }

    @Test
    fun `Should create a tree cell and check toString method`() {
        ...
        assertThat(simpleTree.toString(), allOf(
                containsString("name=Lymphocytes"),
                containsString("easyAccess=0"),
                containsString("listOfChildren=[]")))

    }

    @Test
    fun `Should create a tree cell and check hashCode and equals method`() {

        val simpleTree = Node()
        simpleTree.name = "Lymphocytes"

        val equalTree = Node()
        equalTree.name = "Lymphocytes"

        val differentTree = Node()
        differentTree.name = "Lymphocytes Different"

        assertEquals(equalTree, simpleTree)
        assertNotEquals(differentTree, simpleTree)
        assertFalse(simpleTree.equals(null))
        assertFalse(simpleTree.equals("String"))

        assertEquals(hashCode(simpleTree), simpleTree.hashCode())
